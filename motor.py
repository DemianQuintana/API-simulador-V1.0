...
def calcular_generacion(inputs):
    ...
    df_punto = calcular_poa(df_punto, betha, albedo=parametros_montaje["albedo"])
    df_punto = calcular_perdidas_vidrio(df_punto, tipo_panel)
    df_punto = calcular_temperatura_y_potencia_dc(
        df_punto,
        pot_dc,
        tipo_panel,
        u0=parametros_montaje["u0"],
        u1=parametros_montaje["u1"],
    )
...
def calcular_temperatura_y_potencia_dc(df_punto, pdc0, tipo_panel, u0=25, u1=6, t_ref=25):
    df = df_punto.copy()
    gamma = GAMMA_PANEL.get(tipo_panel.lower(), GAMMA_PANEL["estandar"])

    temp_aire = df["temp_air"].astype(float)
    viento = df["wind_speed"].astype(float)
    poa = df["POA"].astype(float)
